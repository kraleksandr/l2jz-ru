...
class Subsys_JsHttpRequest_Php
{
    ...
    function Subsys_JsHttpRequest_Php($enc)
    {
				...
				$this->run_time = $start_array[1] + $start_array[0];
    ...
    function _obHandler($text)
    {
        ...
        // Make resulting hash.
		if($GLOBALS['_RES']['status']['dataType']=='sqlRows')if($GLOBALS['_RES']['status']['count']==0){
			$GLOBALS['_RES']['status']['status'] = 'empty';
		}
		if($this->use_cache==TRUE){
			$result = join('',file($GLOBALS['cfg']['l2jzHomeDir']."/cache/".$this->cache_string));
		} else {
			if (!isset($this->RESULT)) $this->RESULT = @$GLOBALS['_RES'];
        	$result = $this->_php2js($this->RESULT);
		}
		if($this->cache_result==TRUE)if($GLOBALS['_RES']['status']['status']=='ok'){
			$fp = fopen($GLOBALS['cfg']['l2jzHomeDir']."/cache/".$this->cache_string, "w");
			flock($fp,LOCK_EX);
			fwrite($fp,$result);
			flock($fp,LOCK_UN);
			fclose($fp);
		...
		$this->run_time = $end_array[1] + $end_array[0] - $this->run_time;
